...
import { getAssessmentPrintView, AssessmentPrintViewResponse } from "@/lib/api";
      ...
      <div className="mx-auto max-w-4xl">
        <div className="mb-6 flex items-center justify-between gap-4 print:hidden">
          ...
          <button
            ...
            className="inline-flex min-h-10 items-center justify-center gap-2 rounded bg-brand px-4 text-sm font-semibold text-white shadow-glow hover:bg-brand-500"
          >
              ...
              {question.question_type === "multiple_choice" ? (
                <div className="mt-3 grid gap-2">
                  {question.choices.map((choice) => (
                    <p key={choice} className="text-sm">□ {choice}</p>
                  ))}
                </div>
              ) : (
